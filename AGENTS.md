...
12. **DI・総合検証**  
    ...
    - テスト実行はユーザーから明示的な指示があった場合のみ実施する（デフォルトではスキップ）。  
    - 手動検証が必要なときは、実施内容と結果を共有する。
...
- **テストと検証**  
  - テストコードの追加や `task test`（= `go test -race ./...`）の実行は、ユーザーのリクエストがある場合のみ対応する。  
  - テストを実行しない場合でも、エラーハンドリングや境界ケースをコード上で確認し、必要に応じて手動検証を記録する。
- **コミットとログ**  
