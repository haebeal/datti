...
import { EventEndpoints_EventPutRequest } from "~/api/@types";
...
export function EventUpdateForm({ defaultValue }: Props) {
            ...
            <>
              {paymentFields.map((payment) => (
                <div key={payment.id} className="w-full">
                  <input
                    {...getInputProps(payment.getFieldset().payment_id, {
                      type: "hidden",
                    })}
                    key={payment.getFieldset().payment_id.id}
                  />
                  <Label>
                    {
                      members.find(
