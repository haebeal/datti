...
func (sr *SubscriptionRepositoryImpl) FindByUserIDAndChannel(ctx context.Context, userID string, channel domain.NotificationChannel) (*domain.Subscription, error) {
	...
	subscription, err := domain.NewSubscription(ctx, domain.NotificationChannel(row.Channel), row.EventFiring, row.WeeklySummary)
	if err != nil {
...
func (sr *SubscriptionRepositoryImpl) Upsert(ctx context.Context, userID string, s *domain.Subscription) error {
